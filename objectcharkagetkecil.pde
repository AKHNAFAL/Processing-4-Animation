class ObjectCharkagetkecil {
  ...
  ObjectCharkagetkecil(float x, float y) {
    this.x = x;
    this.y = y;
    this.vx = 0;
    this.vy = 0;
  ...
  void display() {
    ...
    // left
    if (walkFrame == 0) {
      line(x, y + 50, x - 40, y - 110);
    } else if (walkFrame == 1 || walkFrame == 3) {
      line(x, y + 50, x - 30, y - 120);
    } else if (walkFrame == 2) {
    ...
    // Draw Mouth Terkejoet
    fill(#7A0202);
    pushMatrix();
    translate(-20, 30); // Shift right
    ellipse(x, y, 15, 25);
    popMatrix();
